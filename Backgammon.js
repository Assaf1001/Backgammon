...
function isLegalReturnFromJail(selectedPiece){
    if (isWhiteTurn ? (boardRepresentation[selectedPiece].toString().includes('W') && boardRepresentation[0].contain.length>=1): (boardRepresentation[selectedPiece].toString().includes('B') && boardRepresentation[25].contain.length>=1))
        if (isWhiteTurn ? (diceResult.length>=2 ? (25-diceResult[0] === selectedPiece || 25-diceResult[1] === selectedPiece) : 25-diceResult[0] === selectedPiece) :
            (diceResult.length>=2 ? (diceResult[0] === selectedPiece || diceResult[1] === selectedPiece) : diceResult[0] === selectedPiece))
            return true
        else if (!isWhiteTurn ? (diceResult.length>=2 ? (diceResult[0] === selectedPiece || diceResult[1] === selectedPiece) : diceResult[0] === selectedPiece) :
            (diceResult.length>=2 ? (25-diceResult[0] === selectedPiece || 25-diceResult[1] === selectedPiece) : 25-diceResult[0] === selectedPiece))
            return true
     return false
}
...
function isValidPostMove(){
    if (boardRepresentation[(isWhiteTurn ? 0 : 25)].contain.length !== 0){
        for (let i=isWhiteTurn?19:1; isWhiteTurn?i<=24:i<=6; i++)
